...
### 调度节奏抖动（反"固定点/固定间隔"机器指纹）

平台即使不弹警告，也可能通过**调度规律**静默识别自动化：每天同一秒启动、每 30 分钟整点
巡检、同 IP 数秒内连发多个平台，都是强关联信号。调度器内置三层随机抖动（可逐任务关闭）：

- **触发抖动 `jitter_seconds`**：APScheduler 原生 jitter——任务在 cron 触发点后随机延迟
  0~N 秒再执行。`daily_post` 配 `0-900` 会把"每天 09:00 整"打散到 09:00~09:15 之间的
  随机时刻；`maintain_traffic` 配 480 会打散严格每 30 分钟整点的节奏。写法支持整数、
  `"min-max"` 字符串。
- **平台发布错峰 `platform_gap`**：一次 `daily_post` 会按序发多个平台，两个平台之间随机
  等待（默认 180~900 秒，可配 `"min-max"`），避免同 IP 几十秒内连发小红书+抖音这类关联发文。
- **作品巡检间隔 `note_gap`**：`maintain_traffic` 处理多个作品之间随机间隔（默认 15~90 秒），
  模拟"一条条翻看再回复"而不是流水线连刷。
...
daily_post:
  cron: 0 9 * * *
  jitter_seconds: "0-900"     # 触发后随机延迟 0~900s（0 关闭）
  platform_gap: "180-600"     # 平台间随机错峰（秒 / "min-max"）
```

代价：任务不再"准点"，但更接近真人作息。配合上面的拟人行为层（Humanizer），浏览器内动作
与调度节奏两端都去掉了固定节拍。
