...
均匀打字节奏都是强特征）。Agent 内置了可开关的拟人行为噪声层（默认开启，
`HUMANIZER_ENABLED=0` 关闭）：

- **不直奔发布页**：发布前会在首页随机滚动浏览、光标游走/悬停（制造“到场感”），
...
- **自动化痕迹遮蔽**：页面层注入 `navigator.webdriver=false` 等遮蔽（纵深防御）。

代价：单次发布任务会多花约 10~30s（随机）。注意行为拟人 ≠ 100% 不被识别：
